...
export default function EditorTabs({ files, activeFile, onSelectFile }: EditorTabsProps) {
  return (
    <div className="h-9 flex bg-[#252526] border-b border-[#1e1e1e] overflow-x-auto">
      {files.map((file) => (
        <Tab key={file.id} file={file} isActive={file.id === activeFile} onClick={() => onSelectFile(file.id)} />
      ))}
    </div>
  )
}
