...
export const calculateWaveY = (
  ...
  return (
    baseY +
    Math.sin(x * config.frequency * 0.01 + time + layer) *
      (50 * config.amplitude) +
    Math.sin(x * config.frequency * 0.02 + time * 1.2) *
      (30 * config.amplitude) +
    Math.sin(x * config.frequency * 0.003 + time * 0.3) *
      (100 * config.amplitude)
  );
};

export const getWaveColor = (
  time: number,
  layer: number,
  config: WaveConfig,
): string => {
